 ...
 * fiche artiste, pop-up d'un titre...). On s'en sert pour ne fermer que la DERNIERE
 * fenetre ouverte quand on appuie sur le bouton retour, meme si plusieurs sont empilees.
 */
let overlayStack = [];
let listenerAttached = false;
let pendingSyncPops = 0;

function ensureListener() {
  if (listenerAttached || typeof window === "undefined") return;
  listenerAttached = true;
  window.addEventListener("popstate", () => {
    // Cet evenement est juste l'echo d'une fermeture faite via un bouton de l'interface
    // (pas un vrai appui sur le bouton retour) : on l'ignore, la fenetre est deja fermee.
    if (pendingSyncPops > 0) {
      pendingSyncPops -= 1;
      return;
    }
...
export default function useBackButtonClose(isOpen, onClose) {
  ...
  useEffect(() => {
    if (isOpen && !wasOpenRef.current) {
      ...
      // en prevenant le listener global que cet echo ne doit fermer rien d'autre.
      if (wasStillInStack) {
        pendingSyncPops += 1;
        window.history.back();
      }
